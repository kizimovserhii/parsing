...
final class Help
{
    ...
    private const HELP_TEXT   = [
        ...
        'Code Coverage Options' => [
            ['arg' => '--coverage-clover <file>', 'desc' => 'Generate Utilities coverage report in Clover XML format'],
            ['arg' => '--coverage-crap4j <file>', 'desc' => 'Generate Utilities coverage report in Crap4J XML format'],
            ['arg' => '--coverage-html <dir>', 'desc' => 'Generate Utilities coverage report in HTML format'],
            ['arg' => '--coverage-php <file>', 'desc' => 'Export PHP_CodeCoverage object to file'],
            ['arg' => '--coverage-text=<file>', 'desc' => 'Generate Utilities coverage report in text format [default: standard output]'],
            ['arg' => '--coverage-xml <dir>', 'desc' => 'Generate Utilities coverage report in PHPUnit XML format'],
            ['arg' => '--whitelist <dir>', 'desc' => 'Whitelist <dir> for Utilities coverage analysis'],
            ['arg' => '--disable-coverage-ignore', 'desc' => 'Disable annotations for ignoring Utilities coverage'],
            ['arg' => '--no-coverage', 'desc' => 'Ignore Utilities coverage configuration'],
            ['arg' => '--dump-xdebug-filter <file>', 'desc' => 'Generate script to set Xdebug Utilities coverage filter'],
        ],
    ...
    {
        foreach (self::HELP_TEXT as $section => $options) {
            print Help . phpColor::colorize('fg-yellow', "{$section}:") . PHP_EOL;
            ...
            foreach ($options as $option) {
                ...
                if (isset($option['arg'])) {
                    ...
                    for ($i = 1; $i < count($desc); $i++) {
                        print Help . phpstr_repeat(' ', $this->maxArgLength + 3) . $desc[$i] . PHP_EOL;
                    }
                }
            }
