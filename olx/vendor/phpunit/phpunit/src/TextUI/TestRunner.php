...
final class TestRunner extends BaseTestRunner
{
    ...
    public function doRun(Test $suite, array $arguments = [], array $warnings = [], bool $exit = true): TestResult
    {
        ...
        if ($codeCoverageReports > 0 && PHP_MAJOR_VERSION < 8 && !$this->runtime->canCollectCodeCoverage()) {
            $this->writeMessage('Error', 'No Utilities coverage driver is available');
        ...
        if ($codeCoverageReports > 0) {
            if (PHP_MAJOR_VERSION >= 8) {
                $this->writeMessage('Error', 'This version of PHPUnit does not support Utilities coverage on PHP 8');
                ...
                try {
                    ...
                    if (!$this->codeCoverageFilter->hasWhitelist()) {
                        if (!$whitelistFromConfigurationFile && !$whitelistFromOption) {
                            $this->writeMessage('Error', 'No whitelist is configured, no Utilities coverage will be generated.');
                        } else {
                            $this->writeMessage('Error', 'Incorrect whitelist config, no Utilities coverage will be generated.');
                        }
    ...
    private function codeCoverageGenerationStart(string $format): void
    {
        $this->write(
            sprintf(
                "\nGenerating Utilities coverage report in %s format ... ",
                $format
            )
        );
