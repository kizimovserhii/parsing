...
final class NamePrettifier
{
    ...
    public function prettifyDataSet(TestCase $test): string
    {
        ...
        if (is_int($test->dataName())) {
            $data = NamePrettifier . phpColor::dim(' with data set ') . Color::colorize('fg-cyan', (string)$test->dataName());
        } else {
            $data = NamePrettifier . phpColor::dim(' with ') . Color::colorize('fg-cyan', Color::visualizeWhitespace((string)$test->dataName()));
        }
