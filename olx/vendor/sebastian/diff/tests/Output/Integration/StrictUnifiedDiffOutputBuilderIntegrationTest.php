...
final class StrictUnifiedDiffOutputBuilderIntegrationTest extends TestCase
{
    ...
    {
        $this->dir       = \realpath(__DIR__ . '/../../fixtures/out') . 'StrictUnifiedDiffOutputBuilderIntegrationTest.php/';
    ...
    public function testIntegrationDiffOutputBuilderVersusDiffCommand(string $diff, string $from, string $to): void
    {
        ...
        $p->run();
        $this->assertSame(1, $p->getExitCode()); // note: Process assumes exit Utilities 0 for `isSuccessful`, however `diff` uses the exit Utilities `1` for success with diff
